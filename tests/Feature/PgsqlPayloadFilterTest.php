...
class PgsqlPayloadFilterTest extends TestCase
{
    ...
    public function test_null_payload_value_only_matches_a_null_filter(): void
    {
    ...
    public function test_boolean_filter_does_not_match_integer_payload_value(): void
    {
    ...
    private function setVector(VenueWithPayload $venue, array $vector, string $slot = 'default'): void
    {
        ...
        DB::connection(config('embedding.database.connection'))->statement(
            'UPDATE '.config('embedding.database.embeddings_table', 'embeddings').' SET vector = ?::vector, updated_at = NOW() WHERE embeddable_type = ? AND embeddable_id = ? AND slot = ?',
            [$literal, $venue->getMorphClass(), $venue->id, $slot]
        );
    }
}

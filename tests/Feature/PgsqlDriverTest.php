...
{
    public function test_it_returns_sorted_results_using_cosine_distance_operator(): void
    {
    ...
    public function test_zero_threshold_returns_anti_correlated_results(): void
    {
        ...
        $results = Post::similarTo($v1, limit: 10);

        $this->assertCount(2, $results);
        $this->assertEquals($post1->id, $results->first()->id);
        $this->assertEqualsWithDelta(-1.0, $results->last()->similarity_score, 0.0001);
    }
    ...
    private function setVector(Post $post, array $vector, string $slot = 'default'): void
    {
        ...
        DB::connection(config('embedding.database.connection'))->statement(
            ...
            [$literal, $post->getMorphClass(), $post->id, $slot]
        );
    }
}
